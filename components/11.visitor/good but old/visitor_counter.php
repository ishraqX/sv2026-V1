...
// ── Session: safe start (no double-start error) ──────────────
...
if (empty($_SESSION['sv_counted'])) {
    $_SESSION['sv_counted'] = true;
    $sv_geo = sv_get_country($sv_ip);
    $sv_dev = sv_parse_ua($sv_ua);

    // Cache geo in session to avoid repeated API calls
    ...
    $sv_totalCount = sv_get_total($sv_visitorFile);
    $sv_geo        = $_SESSION['sv_geo'] ?? ['country' => 'Unknown', 'countryCode' => 'XX', 'city' => '', 'region' => ''];
}
...
// Show only to localhost (dev) OR to users with sv_admin session set.
// To grant admin access, set $_SESSION['sv_admin'] = true in your login script.
